    ...
    async def analyze_section(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if cached is not None:
                logger.info(f"WebIntelAgent cache hit: {cache_key[:8]}...")
                return cached
        except Exception:
            logger.exception("Cache lookup failed for WebIntelAgent")

        system_msg = (
            "You are a HIGH-PRECISION Web Intelligence Analyst specializing in pharmaceutical web intelligence. "
            "You MUST produce maximum-depth, evidence-driven web intelligence with clear explanations. "
            ...
            "- Tables with Source, Date, Quote/Evidence, Implication, Link\n"
            "- Key insights with category='regulatory' or appropriate category\n"
            "- Citations from PubMed, FDA, EMA, news sources\n\n"
            + get_unified_schema_prompt("WebIntel")
        )

        user_msg = json.dumps({"query": parameters.get("query"), "context": parameters.get("context")})
        ...
        try:
            ...
            logger.error(f"WebIntelAgent LLM failure: {e}")
            result = normalize_to_unified_schema("WebIntel", {}, query)
            result["confidence_score"]["value"] = 0.0
            result["confidence_score"]["explanation"] = f"LLM call failed: {str(e)}"
            result["core_findings"]["summary"] = [f"Error: Web intelligence analysis failed - {str(e)}"]
            return result
        try:
            parsed = json.loads(raw)
        except Exception:
            logger.error("WebIntelAgent parse failure")
            result = normalize_to_unified_schema("WebIntel", {}, query)
            result["confidence_score"]["value"] = 0.0
            result["confidence_score"]["explanation"] = "Failed to parse LLM output"
            result["core_findings"]["summary"] = ["Error: Failed to parse web intelligence analysis"]
            try:
                cache.set(cache_key, result)
            except Exception:
                logger.exception("Failed to cache parse_failure for WebIntelAgent")
            return result

        # Normalize to unified schema
        result = normalize_to_unified_schema("WebIntel", parsed, query)
        
        # Ensure confidence is properly set
        if result["confidence_score"]["value"] == 0.0:
            result["confidence_score"]["value"] = 0.7
            result["confidence_score"]["explanation"] = "Based on web intelligence data availability from PubMed, FDA, EMA, and news sources"

        try:
            cache.set(cache_key, result)
        except Exception:
            logger.exception("Failed to cache WebIntelAgent result")

        logger.info(f"WebIntelAgent completed in {time.time() - start:.2f}s")
        return result
